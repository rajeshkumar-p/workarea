# Simple program to demonstrate XML processing using python

# XML
# 1) XML is a portable, open source language that allows programmers to develop
#    applications that can be read by other applications, regardless of operating
#    system and/or developmental language.
# 2) The Extensible Markup Language (XML) is a markup language much like HTML
# 3) Recommended by the World Wide Web Consortium and available as an
#    open standard.
# 4) XML is extremely useful for keeping track of small to medium amounts of data
#    without requiring a SQL-based backbone.

# XML Parser Architectures and APIs
# 1) Python standard library provides a minimal but useful set of interfaces to
#     work with XML.
# 2) Two most basic and broadly used APIs to XML data are the SAX and DOM
#    interfaces.
#
...
# when your documents are large or you have memory limitations, it parses the file
...
# different APIs literally complement each other, there is no reason why you
# cannot use them both for large projects.
...
Your ContentHandler handles the particular tags and attributes of your flavor(s)
of XML. A ContentHandler object provides methods to handle various parsing
events. Its owning parser calls ContentHandler methods as it parses the XML
file.

The methods startDocument and endDocument are called at the start and the end of
the XML file. The method characters(text) is passed character data of the XML
file via the parameter text.

The ContentHandler is called at the start and end of each element. If the parser
is not in namespace mode, the methods startElement(tag, attributes) and
...
Here is the detail of the parameters −
    ...
    parser_list − The optional argument consisting of a list of parsers to use
...
For a complete detail on SAX API documentation, please refer to standard Python
SAX APIs.
Parsing XML with DOM APIs

The Document Object Model ("DOM") is a cross-language API from the World Wide
Web Consortium (W3C) for accessing and modifying XML documents.
...
element, you have no access to another.
...
The sample phrase calls the parse( file [,parser] ) function of the minidom
object to parse the XML file designated by file into a DOM tree object.
